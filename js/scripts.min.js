...
return;}
if(opts.slideActiveClass){opts.slides.removeClass(opts.slideActiveClass)
...
.eq(opts.currSlide).addClass(opts.pagerActiveClass);});}});$(document).on('cycle-destroyed',function(e,opts){var pager=opts.API.getComponent('pager');if(pager){pager.children().off(opts.pagerEvent);if(opts.pagerTemplate)
...
return prop;}
return str;});}});})(jQuery);(function($){"use strict";$(document).on('cycle-bootstrap',function(e,opts,API){if(opts.fx!=='carousel')
...
opts.carouselVisible=opts.slideCount-1;var visCount=opts.carouselVisible||opts.slides.length;var slideCSS={display:vert?'block':'inline-block',position:'static'};opts.container.css({position:'relative',overflow:'hidden'});opts.slides.css(slideCSS);opts._currSlide=opts.currSlide;wrap=$('<div class="cycle-carousel-wrap"></div>')
.prependTo(opts.container)
.css({margin:0,padding:0,top:0,left:0,position:'absolute'})
.append(opts.slides);opts._carouselWrap=wrap;if(!vert)
...
$(window).off('resize',opts._carouselResizeThrottle);opts.slides.prependTo(opts.container);opts._carouselWrap.remove();}};})(jQuery);;(function($){var $scrollTo=$.scrollTo=function(target,duration,settings){$(window).scrollTo(target,duration,settings);};$scrollTo.defaults={axis:'xy',duration:parseFloat($.fn.jquery)>=1.3?0:1};$scrollTo.window=function(scope){return $(window)._scrollable();};$.fn._scrollable=function(){return this.map(function(){var elem=this,isWin=!elem.nodeName||$.inArray(elem.nodeName.toLowerCase(),['iframe','#document','html','body'])!=-1;if(!isWin)
...
next();});}};});})(jQuery);(function($,undefined){var mouseHandled=false;$(document).mouseup(function(){mouseHandled=false;});$.widget("ui.mouse",{version:"1.10.3",options:{cancel:"input,textarea,button,select,option",distance:1,delay:0},_mouseInit:function(){var that=this;this.element
...
return false;},_mouseDistanceMet:function(event){return(Math.max(Math.abs(this._mouseDownEvent.pageX-event.pageX),Math.abs(this._mouseDownEvent.pageY-event.pageY))>=this.options.distance);},_mouseDelayMet:function(){return this.mouseDelayMet;},_mouseStart:function(){},_mouseDrag:function(){},_mouseStop:function(){},_mouseCapture:function(){return true;}});})(jQuery);(function($,undefined){$.extend($.ui,{datepicker:{version:"1.10.3"}});var PROP_NAME="datepicker",instActive;function Datepicker(){this._curInst=null;this._keyEvent=false;this._disabledInputs=[];this._datepickerShowing=false;this._inDialog=false;this._mainDivId="ui-datepicker-div";this._inlineClass="ui-datepicker-inline";this._appendClass="ui-datepicker-append";this._triggerClass="ui-datepicker-trigger";this._dialogClass="ui-datepicker-dialog";this._disableClass="ui-datepicker-disabled";this._unselectableClass="ui-datepicker-unselectable";this._currentClass="ui-datepicker-current-day";this._dayOverClass="ui-datepicker-days-cell-over";this.regional=[];this.regional[""]={closeText:"Done",prevText:"Prev",nextText:"Next",currentText:"Today",monthNames:["January","February","March","April","May","June","July","August","September","October","November","December"],monthNamesShort:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],dayNames:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],dayNamesShort:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],dayNamesMin:["Su","Mo","Tu","We","Th","Fr","Sa"],weekHeader:"Wk",dateFormat:"mm/dd/yy",firstDay:0,isRTL:false,showMonthAfterYear:false,yearSuffix:""};this._defaults={showOn:"focus",showAnim:"fadeIn",showOptions:{},defaultDate:null,appendText:"",buttonText:"...",buttonImage:"",buttonImageOnly:false,hideIfNoPrevNext:false,navigationAsDateFormat:false,gotoCurrent:false,changeMonth:false,changeYear:false,yearRange:"c-10:c+10",showOtherMonths:false,selectOtherMonths:false,showWeek:false,calculateWeek:this.iso8601Week,shortYearCutoff:"+10",minDate:null,maxDate:null,duration:"fast",beforeShowDay:null,beforeShow:null,onSelect:null,onChangeMonthYear:null,onClose:null,numberOfMonths:1,showCurrentAtPos:0,stepMonths:1,stepBigMonths:12,altField:"",altFormat:"",constrainInput:true,showButtonPanel:false,autoSize:false,disabled:false};$.extend(this._defaults,this.regional[""]);this.dpDiv=bindHover($("<div id='"+this._mainDivId+"' class='ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all'></div>"));}
...
.focus();offset=closestHandle.offset();mouseOverHandle=!$(event.target).parents().addBack().is(".ui-slider-handle");this._clickOffset=mouseOverHandle?{left:0,top:0}:{left:event.pageX-offset.left-(closestHandle.width()/2),top:event.pageY-offset.top-
...
$.Widget.prototype._setOption.apply(this,arguments);switch(key){case"orientation":this._detectOrientation();this.element
...
$.datepicker.regional['de']={monthNames:['Januar','Februar','März','April','Mai','Juni','Juli','August','September','Oktober','November','Dezember'],monthNamesShort:['Jan','Feb','Mär','Apr','Mai','Jun','Jul','Aug','Sep','Okt','Nov','Dez'],dayNames:['Sonntag','Montag','Dienstag','Mittwoch','Donnerstag','Freitag','Samstag'],dayNamesShort:['So','Mo','Di','Mi','Do','Fr','Sa'],dayNamesMin:['So','Mo','Di','Mi','Do','Fr','Sa'],dateFormat:'dd.mm.yyyy',firstDay:1,prevText:'&#x3c;zurück',prevStatus:'letzten Monat zeigen',prevJumpText:'&#x3c;&#x3c;',prevJumpStatus:'',nextText:'Vor&#x3e;',nextStatus:'nächsten Monat zeigen',nextJumpText:'&#x3e;&#x3e;',nextJumpStatus:'',currentText:'heute',currentStatus:'',todayText:'heute',todayStatus:'',clearText:'löschen',clearStatus:'aktuelles Datum löschen',closeText:'schließen',closeStatus:'ohne Änderungen schließen',yearStatus:'anderes Jahr anzeigen',monthStatus:'anderen Monat anzeigen',weekText:'Wo',weekStatus:'Woche des Monats',dayStatus:'Wähle D, M d',defaultStatus:'Wähle ein Datum',isRTL:false};$.datepicker.regional['en']={monthNames:['January','February','March','April','May','June','July','August','September','October','November','December'],monthNamesShort:['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],dayNames:['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'],dayNamesShort:['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],dayNamesMin:['Su','Mo','Tu','We','Th','Fr','Sa'],dateFormat:'dd/mm/yyyy',firstDay:1,prevText:'Prev',prevStatus:'Show the previous month',prevJumpText:'&#x3c;&#x3c;',prevJumpStatus:'Show the previous year',nextText:'Next',nextStatus:'Show the next month',nextJumpText:'&#x3e;&#x3e;',nextJumpStatus:'Show the next year',currentText:'Current',currentStatus:'Show the current month',todayText:'Today',todayStatus:'Show today\'s month',clearText:'Clear',clearStatus:'Erase the current date',closeText:'Done',closeStatus:'Close without change',yearStatus:'Show a different year',monthStatus:'Show a different month',weekText:'Wk',weekStatus:'Week of the year',dayStatus:'Select DD, M d',defaultStatus:'Select a date',isRTL:false};$.datepicker.regional['fr']={monthNames:['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'],monthNamesShort:['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc'],dayNames:['Dimanche','Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi'],dayNamesShort:['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'],dayNamesMin:['Di','Lu','Ma','Me','Je','Ve','Sa'],dateFormat:'dd/mm/yyyy',firstDay:1,prevText:'&#x3c;Préc',prevStatus:'Voir le mois précédent',prevJumpText:'&#x3c;&#x3c;',prevJumpStatus:'Voir l\'année précédent',nextText:'Suiv&#x3e;',nextStatus:'Voir le mois suivant',nextJumpText:'&#x3e;&#x3e;',nextJumpStatus:'Voir l\'année suivant',currentText:'Courant',currentStatus:'Voir le mois courant',todayText:'Aujourd\'hui',todayStatus:'Voir aujourd\'hui',clearText:'Effacer',clearStatus:'Effacer la date sélectionnée',closeText:'Fermer',closeStatus:'Fermer sans modifier',yearStatus:'Voir une autre année',monthStatus:'Voir un autre mois',weekText:'Sm',weekStatus:'Semaine de l\'année',dayStatus:'\'Choisir\' le DD d MM',defaultStatus:'Choisir la date',isRTL:false};$.datepicker.regional['it']={monthNames:['Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'],monthNamesShort:['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'],dayNames:['Domenica','Lunedì','Martedì','Mercoledì','Giovedì','Venerdì','Sabato'],dayNamesShort:['Dom','Lun','Mar','Mer','Gio','Ven','Sab'],dayNamesMin:['Do','Lu','Ma','Me','Gi','Ve','Sa'],dateFormat:'dd/mm/yyyy',firstDay:1,prevText:'&#x3c;Prec',prevStatus:'Mese precedente',prevJumpText:'&#x3c;&#x3c;',prevJumpStatus:'Mostra l\'anno precedente',nextText:'Succ&#x3e;',nextStatus:'Mese successivo',nextJumpText:'&#x3e;&#x3e;',nextJumpStatus:'Mostra l\'anno successivo',currentText:'Oggi',currentStatus:'Mese corrente',todayText:'Oggi',todayStatus:'Mese corrente',clearText:'Svuota',clearStatus:'Annulla',closeText:'Chiudi',closeStatus:'Chiudere senza modificare',yearStatus:'Seleziona un altro anno',monthStatus:'Seleziona un altro mese',weekText:'Sm',weekStatus:'Settimana dell\'anno',dayStatus:'\'Seleziona\' D, M d',defaultStatus:'Scegliere una data',isRTL:false};$.datepicker.regional['ru']={monthNames:['Январь','Февраль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь'],monthNamesShort:['Янв','Фев','Мар','Апр','Май','Июн','Июл','Авг','Сен','Окт','Ноя','Дек'],dayNames:['воскресенье','понедельник','вторник','среда','четверг','пятница','суббота'],dayNamesShort:['вск','пнд','втр','срд','чтв','птн','сбт'],dayNamesMin:['Вс','Пн','Вт','Ср','Чт','Пт','Сб'],dateFormat:'dd.mm.yyyy',firstDay:1,prevText:'&#x3c;Пред',prevStatus:'',prevJumpText:'&#x3c;&#x3c;',prevJumpStatus:'',nextText:'След&#x3e;',nextStatus:'',nextJumpText:'&#x3e;&#x3e;',nextJumpStatus:'',currentText:'Сегодня',currentStatus:'',todayText:'Сегодня',todayStatus:'',clearText:'Очистить',clearStatus:'',closeText:'Закрыть',closeStatus:'',yearStatus:'',monthStatus:'',weekText:'Не',weekStatus:'',dayStatus:'D, M d',defaultStatus:'',isRTL:false};$.datepicker.regional['ua']={monthNames:['Січень','Лютий','Березень','Квітень','Травень','Червень','Липень','Серпень','Вересень','Жовтень','Листопад','Грудень'],monthNamesShort:['Січ','Лют','Бер','Кві','Тра','Чер','Лип','Сер','Вер','Жов','Лис','Гру'],dayNames:['неділя','понеділок','вівторок','середа','четвер','пятниця','субота'],dayNamesShort:['нед','пон','вів','сер','чет','пят','суб'],dayNamesMin:['Нд','Пн','Вв','Ср','Чт','Пт','Сб'],dateFormat:'dd.mm.yyyy',firstDay:1,prevText:'&#x3c;Попер',prevStatus:'',prevJumpText:'&#x3c;&#x3c;',prevJumpStatus:'',nextText:'Наст&#x3e;',nextStatus:'',nextJumpText:'&#x3e;&#x3e;',nextJumpStatus:'',currentText:'Сьогодні',currentStatus:'',todayText:'Сьогодні',todayStatus:'',clearText:'Очистити',clearStatus:'',closeText:'Закрити',closeStatus:'',yearStatus:'',monthStatus:'',weekText:'Ти',weekStatus:'',dayStatus:'D, M d',defaultStatus:'',isRTL:false};
...
if(typeof define==='function'&&define.amd){define(['outlayer/outlayer','get-size/get-size'],masonryDefinition);}else{window.Masonry=masonryDefinition(window.Outlayer,window.getSize);}})(window);$(document).ready(function(){$(document).click(function(){$(".js-datepicker").hide();$(".js-show-calendar").removeClass('is-active');$(".js-choose").parent().removeClass("is-active");$(".js-choose-list").hide();$('.js-select-list').hide();$(".js-select").parent().removeClass("is-active");$(".js-drop-key").parent().removeClass("is-active");$(".js-drop-list").hide();$(".js-tab-key").removeClass("is-active");$(".js-sel-key").removeClass("is-active");$('.js-tab-cont').hide();$('.js-faq-sticker').removeClass("is-visible");});function select(){$(".js-select").each(function(){var select_list=$(this).parent().find(".js-select-list");$(this).click(function(){if($(this).parent().hasClass("is-active")){$(this).parent().removeClass("is-active");select_list.hide();}
else{$(this).parent().addClass("is-active");select_list.show();}});select_list.find("li").click(function(){var id=$(this).attr("data-id");var text=$(this).text();$(this).parent().parent().find(".js-select").text(text);$(this).parent().parent().find("input").val(id);$(this).parent().hide();$(this).parent().parent().removeClass("is-active");});});}
select();$('.js-select-list').click(function(event){event.stopPropagation();});$('.js-select').click(function(event){event.stopPropagation();});$(".js-sel-key").click(function(event){if($(this).hasClass("is-active")){$(this).removeClass("is-active");}
else{$(".js-sel-key").removeClass("is-active");$(this).addClass("is-active");}
$('.js-show-calendar').removeClass("is-active");$(".js-datepicker").hide();event.stopPropagation();});$(".js-sel-list li").click(function(event){var id=$(this).attr("data-id");var text=$(this).text();$(this).parent().parent().find(".js-sel-text").val(text);$(this).parent().parent().find(".js-sel-input").val(id);$(this).parent().parent().removeClass("is-active");if($(this).parent().parent().hasClass('select_city')){$(".js-datepicker-from").show();var maxpersons=($(this).attr("data-maxpersons")>10)?10:parseInt($(this).attr("data-maxpersons"));$('#dbv_select_guests ul li').each(function(){if(parseInt($(this).attr('data-id'))>maxpersons){$(this).hide();}
else{$(this).show();}});if(maxpersons<$('#dbv_select_guests .js-sel-input').val()){$('#dbv_select_guests .js-sel-text').val('');$('#dbv_select_guests .js-sel-input').val(1);}}
event.stopPropagation();});$(".js-all-city").click(function(){$(".js-city-list").toggle();$('html, body').animate({scrollTop:$(".js-city-list").offset().top},500);return false;});$(".js-to-comments").click(function(){$('html, body').animate({scrollTop:$(".js-comment-target").offset().top},500);return false;});$(".js-to-top").click(function(){$('html, body').animate({scrollTop:0},200);});if($(".js-datepicker").length>0){$(".js-datepicker").each(function(){$(this).hide();$.datepicker.regional['all']={weekHeader:'Не',dateFormat:'yy-mm-dd',firstDay:1,isRTL:false,minDate:'0',showMonthAfterYear:false,yearSuffix:''};$.datepicker.setDefaults($.datepicker.regional[dbv_get_current_lang()]);$.datepicker.setDefaults($.datepicker.regional['all']);$(".js-datepicker-from").datepicker({showOtherMonths:true,onSelect:function(selectedDate){$(".js-show-calendar").removeClass("is-active");$(".js-datepicker").hide();var dateObject=new Date($(".js-datepicker-from").datepicker('getDate'));$(".js-from").val($.datepicker.formatDate('d MM',dateObject));$(".js-from").css('padding-left','10px');$(".js-from").parent().parent().find('.select__input-text').hide();dateObject.setDate(dateObject.getDate()+1);$(".js-to").val($.datepicker.formatDate('d MM',dateObject));$(".js-to").css('padding-left','10px');$(".js-datepicker-to").datepicker("option","minDate",dateObject);$(".js-datepicker-to").datepicker('setDate',dateObject);check_date();$(".js-datepicker-to").show();}});$(".js-datepicker-to").datepicker({showOtherMonths:true,onSelect:function(selectedDate){$(".js-show-calendar").removeClass("is-active");$(".js-datepicker").hide();var dateObject=new Date($(".js-datepicker-to").datepicker('getDate'));$(".js-to").val($.datepicker.formatDate('d MM',dateObject));$(".js-to").css('padding-left','10px');$(".js-to").parent().find('.select__input-text').hide();dateObject.setDate(dateObject.getDate()-1);$(".js-datepicker-from").datepicker("option","maxDate",dateObject);check_date();$(".js-sel-key").removeClass("is-active");$("#dbv_search_btn").fadeTo(100,0.5,function(){$('#dbv_search_btn').fadeTo(100,1);$("#dbv_search_btn").fadeTo(100,0.5,function(){$('#dbv_search_btn').fadeTo(100,1);});});}});});$(".js-show-calendar").click(function(){if($(this).hasClass("is-active")){$(this).removeClass("is-active");$(".js-datepicker").hide();}
else{$(".js-show-calendar").removeClass("is-active");$(this).addClass("is-active");$(".js-datepicker").hide();$(this).find(".js-datepicker").show();}});$('.js-datepicker').click(function(event){event.stopPropagation();});$('.js-show-calendar').click(function(event){$(".js-sel-key").removeClass("is-active");event.stopPropagation();});}
function check_date(){$(".js-flat-pricer").each(function(i){var input_from=$(this).find(".js-from").val().length;var input_to=$(this).find(".js-to").val().length;if(input_from>0&&input_to>0){$(".js-order-btn").show();$(".js-choose-date-btn").hide();}
$(this).find(".js-choose-date-btn").bind("click",function(){$(this).parent().find(".js-first-date").addClass("is-active");$(this).parent().find(".js-first-date .js-datepicker").show();event.stopPropagation();});});}
check_date();$('.js-top-popup').click(function(){$('.js-top-popup').removeClass('is-active');$(this).toggleClass('is-active');});function choose_number(){var number=$(".js-choose-number");number.each(function(){var max_number=+($(this).parent().parent().attr("data-max-number"));var plus=$(this).parent().parent().find(".js-plus");var minus=$(this).parent().parent().find(".js-minus");plus.bind("click",function(){var val=+($(this).parent().find(number).text());if(val>=max_number){return false}
else{val+=1;$(this).parent().find(".choose__input").val(val);$(this).parent().find(number).text(val);}});minus.bind("click",function(){var val=+($(this).parent().find(number).text());if(val>1){val-=1;$(this).parent().find(".choose__input").val(val);$(this).parent().find(number).text(val);}
else{return false;}});});$(".js-choose-key").bind("click",function(){$(this).parent().toggleClass("is-active");$(this).parent().find(".js-choose-list").toggle();});$('.js-choose-list').click(function(event){event.stopPropagation();});$('.js-choose').click(function(event){event.stopPropagation();});$(".js-choose").each(function(){var choose_list=$(this).parent().find(".js-choose-list");$(this).click(function(){if($(this).parent().hasClass("is-active")){$(this).parent().removeClass("is-active");choose_list.hide();}
else{$(".js-choose").parent().removeClass("is-active");$(this).parent().addClass("is-active");$(".js-choose-list").hide();choose_list.show();}});choose_list.find("li").click(function(){var id=$(this).attr("data-id");var text=$(this).text();$(this).parent().parent().find(".js-choose-number").text(text);$(this).parent().parent().find("input").val(id);$(this).parent().hide();$(this).parent().parent().removeClass("is-active");});});}
choose_number();$(".js-extend-link").click(function(){$(this).parent().find(".js-extend").toggleClass("is-active");});function drop_list(){$(".js-drop-key").bind("click",function(event){var drop_id=$(this).attr("data-drop-id");$(this).parent().toggleClass("is-active");$("."+drop_id).toggle();event.stopPropagation();});$('.js-drop-list').click(function(event){event.stopPropagation();});var more=$(".js-drop-more");var hide=$(".js-drop-hide");more.bind("click",function(){$(this).parent().parent().addClass("is-extend");});hide.bind("click",function(){$(this).parent().parent().removeClass("is-extend");});}
drop_list();function init_cycle(){if($(".js-slider").length>0){$(".js-slider").each(function(){var slider_1=$(this).find('.js-cycle-1');var slider_2=$(this).find('.js-cycle-2');var prev_nav=$(this).find('.js-cycle-prev');var next_nav=$(this).find('.js-cycle-next');slider_2.cycle({prev:prev_nav,next:next_nav});slider_1.cycle();var slideshows=$(this).find('.js-cycle').on('cycle-next cycle-prev',function(e,opts){slideshows.not(this).cycle('goto',opts.currSlide);});slider_2.find(".cycle-slide").click(function(){var index=slider_2.data('cycle.API').getSlideIndex(this);slideshows.cycle('goto',index);});});}}
init_cycle();$(".js-close").bind("click",function(){$(this).parent().hide();$(".js-overlay").hide();});$('.js-popup').click(function(event){event.stopPropagation();});$(".js-overlay").bind("click",function(){$(this).hide();$('.js-popup').hide();});function tab(){$(".js-tab-key").bind("click",function(event){if($(this).hasClass("is-active")){$(this).removeClass("is-active");$(".js-tab-cont").hide();}
else{$(".js-tab-key").removeClass("is-active");$(".js-tab-cont").hide();$(this).addClass("is-active");var id=$(this).attr("data-tab");$("."+id).show();}
event.stopPropagation();});$('.js-tab-cont').click(function(event){event.stopPropagation();});}
tab();var tooltip=$(".js-tooltip");tooltip.hover(function(){$(this).show();},function(){$(this).hide()});$(".js-with-tooltip").hover(function(){var left=$(this).offset().left;var top=$(this).offset().top+$(this).outerHeight();var html=$(this).attr("data-info");tooltip.children().html(html);tooltip.css({left:left,top:top});if($(this).parent().parent().hasClass("owner__list")){tooltip.css({left:left-22,top:top});}
tooltip.fadeIn("fast");},function(){tooltip.hide()});$(".js-to-map-popup").click(function(){$(".js-popup-map").show();$(".js-overlay").show();return false;});$(".js-del").click(function(){$(this).parent().remove();});$(".js-marked-link").hide();$(".js-btn-mark").bind("click",function(){$(this).toggleClass("is-active");if($(this).parents(".catalog-wrap").find(".js-btn-mark.is-active").length>0||$(this).parents(".map").find(".js-btn-mark.is-active").length>0||$(this).parents(".flat-pricer").find(".js-btn-mark.is-active").length>0){$(".js-marked-link").show();}
else{$(".js-marked-link").hide();}
return false;});function fixed_flat_info(){if($(".js-targe-info").length){var top=$(".js-targe-info").offset().top;}
if($(window).scrollTop()>top){$(".flat-info-fixed").fadeIn("fast");}
else{$(".flat-info-fixed").fadeOut("fast");}}
if($(".flat-info-fixed").length>0){fixed_flat_info();}
$(window).scroll(function(){if($(".flat-info-fixed").length>0){fixed_flat_info();}});$(".js-open-faq").click(function(event){$(".js-faq-sticker").toggleClass("is-visible");event.stopPropagation();return false;});$(".js-faq-sticker").click(function(event){event.stopPropagation();});$(".js-close-sticker").click(function(){$(this).parent().toggleClass("is-visible");});});
...
return false;};}));
$(document).ready(function(){dbv_load_currency_rates();display_seo_text();lang_propose();dbv_livechat_init();dbv_get_calendar('current');});function display_seo_text(){if($('#dbv_seo_text').length&&$('#dbv_seo_block').length){$('#dbv_seo_block').html($('#dbv_seo_text').html()).show();}}
function lang_propose(){if($.cookie('DOBOVO_OLDUSER')){return;}
$.cookie('DOBOVO_OLDUSER',1,{expires:10,path:'/',});var siteLang=$('#dbv_lang_selector').parent().find('.currency__text').text().toUpperCase();var userLang=navigator.language||navigator.userLanguage;userLang=userLang.substr(0,2).toUpperCase();if(siteLang==userLang){return;}
var html;$('#dbv_lang_selector>li').each(function(){var lang_code=$('a',this).text();if(lang_code!=userLang){return;}
var text=$(this).attr('title');var lang_name=text.split(' ').pop();text=text.replace(/\S+\s*$/,'');html=text+'<a href="'+$('a',this).attr('href')+'">'+lang_name+'</a>';});if(html.length>0){$('body').prepend('<div id=dbv_lang_propose></div>');$('#dbv_lang_propose').html('<button onClick="$(this).parent().hide()"></button>'+html).slideDown(1000);}}
function dbv_change_currency(setCur){var currentCur=$.cookie('Currency')||'uah';currentCur=currentCur.toUpperCase();var newCur=(setCur)?setCur:currentCur;if(newCur!=currentCur||!setCur){$.cookie('Currency',newCur.toLowerCase(),{expires:10,path:'/',});$('.dbv_price').each(function(){if(!$('.dbv_orig',this).length){$('.dbv_val',this).after('<span class=dbv_orig style="display: none">'+$('.dbv_val',this).text()+'</span>');}
$('.dbv_val',this).text(dbv_currency_convert('UAH',newCur,$('.dbv_orig',this).text()));$('.dbv_cur',this).text(newCur);});$('.dbv_price_round').each(function(){if(!$('.dbv_orig',this).length){$('.dbv_val',this).after('<span class=dbv_orig style="display: none">'+$('.dbv_val',this).text()+'</span>');}
$('.dbv_val',this).text(dbv_currency_convert('UAH',newCur,$('.dbv_orig',this).text(),0));$('.dbv_cur',this).text(newCur);});}
$('#current_currency').text(newCur);}
function dbv_currency_convert(from,to,sum,round){if(typeof round=='undefined'){round=2;}
var res=Math.round(sum*(window.dbv_CurrencyRates[from]/window.dbv_CurrencyRates[to])*100)/100;return res.toFixed(round);}
function dbv_load_currency_rates(){var $url=dbv_get_static_url()+'/currencyRates.js';$.getScript($url,function(){dbv_change_currency();});}
function dbv_get_static_url(){var str=$('#dbv_main_js').attr('src').split('/');str.pop();return str.join('/');}
function dbv_loading_img(){return'<img style="padding: 28px" src="'+dbv_get_static_url()+'/../img/loading.gif">';}
function dbv_loading_html(){return'<div style="text-align: center">'+dbv_loading_img()+'</div>'}
function dbv_load_home_city(city_id){var cur_id=city_id.shift();if(!cur_id){return;}
$('.dbv_city_blk:last .dbv_next_btn .load-more').replaceWith(dbv_loading_img());$('.dbv_city_blk:last .dbv_next_btn').css('text-align','center');$.get('/dobovo/home/city.php?cmd=home&show_next_btn=1&lang='+dbv_get_current_lang()+'&city_id='+cur_id,function(data){$('.dbv_city_blk:last .dbv_next_btn').hide();$('.dbv_city_blk:last').after(data);var $container=$('.dbv_city_blk:last>.js-masonry:first');$container.masonry($.parseJSON($container.attr('data-masonry-options')));dbv_change_currency();dbv_load_home_city(city_id);});}
function dbv_get_reviews(type,apt_id,order,page){$('#dbv_'+type+'_review_blk').html(dbv_loading_html());$.get('/dobovo/apt/reviews.php?cmd=apt&type='+type+'&lang='+dbv_get_current_lang()+'&order='+order+'&apt_id='+apt_id,function(data){$('#dbv_'+type+'_review_blk').html(data);});}
function dbv_search_apt(field){var search=$(field).val();if(search.length>0)
{jQuery.getJSON("/tools/ajax.php",{search_apt:search},function(data)
{if(!data.apt_id)
{if(data.hasOwnProperty('message')){show_ajax_error(data.message);}
jQuery(field).val('');}
else
{window.location.href=data.apt_id;}});}else{jQuery(field).val('');}}
function dbv_get_current_lang(){return $('#dbv_lang_selector').parent().find('.currency__text').text();}
function dbv_search(btn){var search_form=$(btn).parent();var city_id=search_form.find('.select_city .js-sel-input').val();$.get('/tools/ajax.php?get_city_url='+city_id,{},function(data){if(data){var date_from=($('.js-from').val().length)?$(".js-datepicker-from").datepicker({dateFormat:'yy-mm-dd'}).val():null;var date_to=($('.js-to').val().length)?$(".js-datepicker-to").datepicker({dateFormat:'yy-mm-dd'}).val():null;var guests=search_form.find('.select_guest .js-sel-input').val();var params='';if(date_from&&date_to){params+='in='+date_from+'&out='+date_to;}
if(parseInt(guests)>1){if(params){params+='&';}
params+='persons='+guests;}
if(params.length){params='?'+params;}
location.href=data+params;}});}
var dbv_aptCalendar;var dbv_aptCalendarVars=new Object;function dbv_get_calendar(direction){data=new Object;data.id=parseInt($('#dbv_apt_id').text());if(typeof dbv_aptCalendar==='object'){if(direction=='next'){dbv_aptCalendar.first_day.setMonth(dbv_aptCalendar.first_day.getMonth()+1);}else{dbv_aptCalendar.first_day.setMonth(dbv_aptCalendar.first_day.getMonth()-1);}
data.date=dbv_DateToSQLFormat(dbv_aptCalendar.first_day);}
$('.dbv_calendar_popup').show();$.ajax({dataType:"json",url:'/dobovo/apt/ajax.php?action=getCalendar'+'&lang='+dbv_get_current_lang(),data:data,method:'POST',error:function(Rdata,Status){$('.dbv_calendar_popup').hide();show_ajax_error("We're sorry! Request has failed: "+Status);return;},success:function(Rdata){$('.dbv_calendar_popup').hide();if(Rdata.hasOwnProperty('message')){show_ajax_error(Rdata.message);$('.respond[item-id='+review_id+']').find('.respond__like>span').hide();}
if(Rdata.success==true){dbv_aptCalendar=Rdata;dbv_update_apt_calendar();return;}}});}
function dbv_update_apt_calendar(){$('#dbv_calendar_first_month').html(dbv_aptCalendar.first_month);$('#dbv_calendar_last_month').html(dbv_aptCalendar.last_month);for(var first_day in dbv_aptCalendar.calendar)break;dbv_aptCalendar.first_day=dbv_DateClassFromSQL(first_day);dbv_aptCalendar.first_day.setDate(1);var today=new Date();today.setHours(0,0,0,0);if(typeof dbv_aptCalendar.discounts!=='undefined'&&$('#dbv_calendar_discounts').html().length==0){var html='';for(i in dbv_aptCalendar.discounts){var val=dbv_aptCalendar.discounts[i];html+='<div data-value='+val.discount+'><span>'+val.days+' '+val.text+'</span> -'+val.discount+'%</div>';}
$('#dbv_calendar_discounts').html(html);$('#dbv_calendar_discounts>div').click(function(){if($(this).hasClass('is-active')){$('#dbv_calendar_discounts>div').removeClass('is-active');dbv_aptCalendarVars.discount=0;}else{$('#dbv_calendar_discounts>div').removeClass('is-active');$(this).addClass('is-active');dbv_aptCalendarVars.discount=$(this).attr('data-value');}
dbv_update_apt_calendar();});}
for(var i=0;i<2;i++){var start=dbv_DateClassFromSQL(first_day);start=new Date(start.getFullYear(),start.getMonth()+i,1);var used_cells=start.getDay()-1;if(used_cells<0){used_cells=6;}
var html='';for(var k=0;k<used_cells;k++)html+='<div class="cell is-other-month"></div>';var end=new Date(start);end.setMonth(end.getMonth()+1);var day=new Date(start);while(day<end){var cell_class='is-last-days';var cur_date=dbv_DateToSQLFormat(day);var date_html=new Array();date_html.push('<a href="#">');date_html.push('<span>'+day.getDate()+'</span>');if(dbv_aptCalendar.calendar[cur_date]){cell_class=(dbv_aptCalendar.calendar[cur_date].state<1)?'is-inactive':'';var price=(typeof dbv_aptCalendarVars.discount==='undefined')?dbv_aptCalendar.calendar[cur_date].price:dbv_aptCalendar.calendar[cur_date].price*(100-dbv_aptCalendarVars.discount)/100;date_html.push('<i class="dbv_price_round"><span class=dbv_cur>UAH</span><span class=dbv_val>'+price+'</span></i>');}
date_html.push('</a>');if(dbv_DateToSQLFormat(day)==dbv_DateToSQLFormat(today)){cell_class+=' is-today';}
if(day<today){cell_class='is-last-days';}
html+='<div class="cell '+cell_class+'">'+date_html.join('')+'</div>';day.setTime(day.getTime()+(24*60*60*1000));used_cells++;}
for(var k=used_cells;k<42;k++)html+='<div class="cell is-other-month"></div>';$('#dbv_calendar_month'+i).html(html);};dbv_change_currency();}
function dbv_DateToSQLFormat(date){return('000'+date.getFullYear()).slice(-4)+'-'+('0'+(1+date.getMonth())).slice(-2)+'-'+('0'+date.getDate()).slice(-2);}
function dbv_DateClassFromSQL(date){date=date.split('-');return new Date(date[0],date[1]-1,date[2]);}
function dbv_review_like(review_id){data=new Object;data.id=review_id;$.ajax({dataType:"json",url:'/dobovo/apt/ajax.php?action=reviewLike',data:data,method:'POST',error:function(Rdata,Status){show_ajax_error("We're sorry! Request has failed: "+Status);return;},success:function(Rdata,Status){if(Rdata.message){show_ajax_error(Rdata.message);$('.respond[item-id='+review_id+']').find('.respond__like>span').hide();}
if(Rdata.success==true){likes_el=$('.respond[item-id='+review_id+']').find('.respond__like>i');likes_el.html(parseInt(likes_el.text())+1);$('.respond[item-id='+review_id+']').find('.respond__like>span').hide();}}});}
function show_ajax_error(text){var el_id='JSmessage_'+parseInt(Math.random()*1000000);$('body').prepend('<div class=dbv_JS_message id='+el_id+'></div>');$('#'+el_id).html('<button onClick="$(this).parent().hide()"></button>'+text).slideDown(500).delay(2000).fadeOut(500,function(){$(this).remove()});}
var __lc={};var __lc_buttons=[];__lc.license=1039501;__lc.dbv_chatID='LiveChat_1390310997';function dbv_livechat_init(){__lc_buttons.push({elementId:__lc.dbv_chatID,skill:'0',window:{width:500,height:520},type:'text',labels:{online:$('#'+__lc.dbv_chatID).attr('data-online'),offline:$('#'+__lc.dbv_chatID).attr('data-online')}});var src=('https:'==document.location.protocol?'https://':'http://')+'cdn.livechatinc.com/tracking.js';$.getScript(src);}
